...
export const ReasoningPanel = memo(
  ({ reasoningResponse, isLoading }: ReasoningPanelProps) => {
    const { expandedPanels, togglePanel, activeCaseId } = useCaseContext();
    const [expandedDifferential, setExpandedDifferential] = useState<
      number | null
    >(null);
    ...
    const primaryDx = useMemo(
      () =>
        hasRecommendations
          ? reasoningResponse?.differentials?.[0] || null
          : null,
      [hasRecommendations, reasoningResponse?.differentials]
    );

    const otherDx = useMemo(
      () =>
        hasRecommendations
    ...
    const progressionMode = useMemo(
      () => reasoningResponse?.progressionMode || "MONITOR",
      [reasoningResponse?.progressionMode]
    );

    const modeStyle = useMemo(
      () => progressionConfig[progressionMode] || progressionConfig["MONITOR"],
      [progressionMode]
    );

    // Memoize differential toggle handler
    const handleDifferentialToggle = useCallback((idx: number) => {
      setExpandedDifferential((prev) => (prev === idx ? null : idx));
    }, []);
              ...
              <div className="space-y-2">
                ...
                <div className="grid gap-1.5">
                  {otherDx.map((dx, idx) => (
                    <div key={idx}>
                      <button
                        onClick={() => handleDifferentialToggle(idx)}
