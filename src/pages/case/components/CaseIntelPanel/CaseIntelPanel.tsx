...
import { CaseIntelPanelSkeleton } from "@/components/ui/skeletons";
...
export const CaseIntelPanel = memo(
  ({ bundle, nextSteps, isLoading }: CaseIntelPanelProps) => {
    ...
    const handleImplicationsToggle = useCallback(() => {
      setImplicationsOpen((prev) => !prev);
    }, []);

    // Extract data from bundle (may be null)
    const patient = bundle?.patient;
    const caseInfo = bundle?.case;
    const flags = bundle?.flags;
    const imaging = bundle?.imaging;
    const specialistInput = bundle?.specialistInput;
    ...
    // Format patient demographics string with null-safety
    ...
    const status = useMemo(
      () => statusConfig[caseStatus] || statusConfig["New"],
      [caseStatus]
    );
    ...
    return (
      ...
      >
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-xl font-bold text-[#F2F2F2]">{patientName}</h2>
        ...
        {specialistInput && (
          <div className="mt-4 border border-[#2A2F33] rounded-lg overflow-hidden">
            <button
              onClick={handleImplicationsToggle}
...
CaseIntelPanel.displayName = "CaseIntelPanel";
